 ...
 * 回流用户数
 */
public class backflowETLMain {

    public static HashMap backflowmain(String toDate) {
        Date date = DateUtil.parse(toDate);
        DateTime profilesDate1 = DateUtil.offsetDay(date, 0);
         ...
         * 1、回流用户数
         */
        HashMap<String, String> allmap = welETL.backflows(toDate,profilesDate);
        ...
        if(now >= newnow){
            welETL.backflowsETL(toDate,toDate,profilesDate,allmap);
        }
        ...
        if(now >= new1now){
            ...
            welETL.backflowsETL(toDate,toDateNew1,profilesDate,allmap);
        }
        ...
        if(now >= new3now){
            ...
            welETL.backflowsETL(toDate,toDateNew3,profilesDate,allmap);
        }
        ...
        if(now >= new5now){
            ...
            welETL.backflowsETL(toDate,toDateNew5,profilesDate,allmap);
        }
        ...
        if(now >= new7now){
            ...
            welETL.backflowsETL(toDate,toDateNew7,profilesDate,allmap);
        }
        ...
        if(now >= new14now){
            ...
            welETL.backflowsETL(toDate,toDateNew14,profilesDate,allmap);
        }
        ...
        if(now >= new30now){
            ...
            welETL.backflowsETL(toDate,toDateNew30,profilesDate,allmap);
        }
...
//         * sink - csv文档.
//         */
//        CSVUtil.createbCSVFile(allmap,outPutPath,"渠道回流");
